...
Pnet::Pnet() {
	...
	if (!if_stream)
	{
		std::cout << "Missing model file:" << filename << std::endl;
		exit(-1);
	}
	else
		if_stream.close();
...
void mtcnn::findFace(Mat& image) {
	...
	for (vector<struct Bbox>::iterator it = thirdBbox_.begin(); it != thirdBbox_.end(); it++) {
		if ((*it).exist) {
			...
			{
				//从上到下，从左到右顺序
				circle(image, Point((int) * (it->ppoint + num), (int) * (it->ppoint + num + 5)), 1, Scalar(0, 255, 255), -1);
				std::cout << Point((int) * (it->ppoint + num), (int) * (it->ppoint + num + 5)) << std::endl;
			}
		}
	}
	firstBbox_.clear();
...
void mtcnn::findFace(const Mat& img, std::vector<std::vector<cv::Point2f>>& landmarks, std::vector<cv::Rect>& rects)
{
	Mat image = img.clone();
	...
	for (vector<struct Bbox>::iterator it = thirdBbox_.begin(); it != thirdBbox_.end(); it++) {
		if ((*it).exist) {
			//rectangle(image, Point((*it).y1, (*it).x1), Point((*it).y2, (*it).x2), Scalar(0,0,255), 2,8,0);
			//for(int num=0;num<5;num++)circle(image,Point((int)*(it->ppoint+num), (int)*(it->ppoint+num+5)),3,Scalar(0,255,255), -1);
			cv::Rect rect(Point((*it).y1, (*it).x1), Point((*it).y2, (*it).x2));
			...
			for (int num = 0; num < 5; num++)
			{
				...
				circle(image, Point((int) * (it->ppoint + num), (int) * (it->ppoint + num + 5)), 1, Scalar(0, 255, 255), -1);
				//std::cout << Point((int)*(it->ppoint + num), (int)*(it->ppoint + num + 5)) << std::endl;
				landmark.push_back(Point((int) * (it->ppoint + num), (int) * (it->ppoint + num + 5)));
			}
			landmarks.push_back(landmark);
			//std::cout << landmarks.at(0) << endl;
		}
	}
	firstBbox_.clear();
	firstOrderScore_.clear();
